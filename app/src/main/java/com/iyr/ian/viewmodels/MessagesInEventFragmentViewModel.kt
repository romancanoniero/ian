...
class MessagesInEventFragmentViewModel : ViewModel() {
    ...
    private var chatFlowJob: Job? = null

    private var _eventKey: String? = null
    ...
    fun onNewSpeedMessage(message: Message) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            if (call.data != null) {
                ...
                _chatEvents.postValue(chatEventsArray)
                chatRepository.sendSpeedMessage(
                    _eventKey.toString(),
...
*/
    ...
    suspend fun onNewMediaMessage(sender: User, mediaFile: MediaFile) {
        ...
        if (call.data != null) {
            ...
            _storingMessage.postValue(Resource.Loading(previewMessage.id))
