...
*/
class SetupFragment : Fragment(), SetupActivityCallback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        args.bundle?.let { bundle ->
            viewModel.setExtraData(bundle)
        }

        binding =
            DataBindingUtil.setContentView(requireActivity(), R.layout.activity_setup)
    ...
    private fun setupObservers() {
        ...
        viewModel.viewStatus.observe(this) { status ->
            when (status) {
                ...
                is Resource.Success -> {
                    loader.dismiss()
                    Toast.makeText(
                        requireContext(), "Setup Updated Successfully", Toast.LENGTH_SHORT
                    ).show()
        ...
        viewModel.displayNameMinCharsRequired.observe(this) { complete ->
            if (!complete) {
                binding.displayNameMinChars.setTextColor(requireContext().getColor(R.color.red))
                binding.displayNameMinChars.visibility = VISIBLE
            } else {
                binding.displayNameMinChars.visibility = GONE
                binding.displayNameMinChars.setTextColor(requireContext().getColor(R.color.black))
            }
        }
        ...
        viewModel.authToken.observe(this) { token ->
            SessionForProfile.getInstance(requireContext()).setProfileProperty("auth_token", token)
        }
                ...
                is Resource.Success -> {
                    ...
                    if (mediaFile != null && !mediaFile.file_name.isNullOrEmpty()) {
                        ...
                        lifecycleScope.launch(Dispatchers.Main) {
                            ...
                            GlideApp.with(requireContext()).asBitmap().load(finalPath)
                                .placeholder(requireContext().getDrawable(R.drawable.progress_animation))
                                .error(requireContext().getDrawable(R.drawable.ic_error))
                                .into(binding.userImage)
    ...
    fun setupUI() {
        ...
        binding.displayNameMinChars.text = String.format(getString(R.string.min_chars_required), AppConstants.USERNAME_MINIMUM_LENGTH
        )
...
//                Intent(this@SetupActivity, PressOrTapSetupActivity::class.java)
            val action =
                SetupFragmentDirections.actionSetupFragmentToPressOrTapSetupFragment(bundle)
            findNavController().navigate(action)
        } else if (user?.security_code.isNullOrBlank()) {
            val action =
                SetupFragmentDirections.actionSetupFragmentToPinSetupFragment(bundle)
            findNavController().navigate(action)
//                Intent(this@SetupActivity, PinSetupActivity::class.java)
            ...
            val action =
                SetupFragmentDirections.actionSetupFragmentToSpeedDialSetupFragment(bundle)
            findNavController().navigate(action)
        } else if (!requireActivity().areLocationPermissionsGranted()) {
            //Intent(this@SetupActivity, LocationRequiredActivity::class.java)
            val action =
                SetupFragmentDirections.actionSetupFragmentToLocationRequiredFragment(bundle)
            findNavController().navigate(action)
        } else {
//                Intent(this@SetupActivity, AddContactsFromPhoneActivity::class.java)
            val action =
                SetupFragmentDirections.actionSetupFragmentToAddContactsFromPhoneFragment(bundle)
            findNavController().navigate(action)
        }
        /*
